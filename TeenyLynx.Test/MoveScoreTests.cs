...
public class MoveScoreTest : BaseTest
{
    ...
    public void MoveScore(string fen)
    {
        ...
        var allMoves = bot._position.GetLegalMoves().OrderByDescending(move => bot.Score(move, default, default, default)).ToList();
        ...
        {
            Assert.AreEqual(0, bot.Score(move, default, default, default));
        }
    }
    ...
    public void MoveScoreEnPassant(string fen, string moveWithHighestScore)
    {
        ...
        Assert.AreEqual(100_000 + new MyBot().Magic[441 + (int)PieceType.Pawn + 6 * (int)PieceType.Pawn], bot.Score(allMoves[0], default, default, default));
    }
}
